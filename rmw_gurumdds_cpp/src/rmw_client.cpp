  ...
  bool * taken)
{
  *taken = false;
  ...
  if (service_mapping_basic) {
    ...
    if (sample_info->valid_data && ros_response != NULL) {
...
static void
client_callback(const dds_DataReader * a_reader) {
  ...
  rmw_ret_t rmw_ret = _take_response(data->client, nullptr, nullptr, &taken);
  if (rmw_ret != RMW_RET_OK) {
    RMW_SET_ERROR_MSG("_take_response failed");
    return;
  }
  ...
  const rmw_qos_profile_t * qos_policies)
{
  ...
  client_info->user_callback_data.client = rmw_client;
  client_info->listener.on_data_available = client_callback;  
  dds_DataReader_set_listener_context(client_info->response_reader, &client_info->user_callback_data);
...
rmw_take_response(
  ...
  bool * taken)
{
  rcutils_log(nullptr, RCUTILS_LOG_SEVERITY_INFO, "rmw_client.cpp", "rmw_take_response");
  RMW_CHECK_ARGUMENT_FOR_NULL(client, RMW_RET_INVALID_ARGUMENT);
  RMW_CHECK_TYPE_IDENTIFIERS_MATCH(
    client handle,
...
{
  auto cli = static_cast<GurumddsClientInfo *>(rmw_client->data);
  GurumddsUserCallback * data = &(cli->user_callback_data);
  dds_ReturnCode_t ret;

  std::lock_guard<std::mutex> guard(data->mutex);
  ret = dds_DataReader_set_listener(cli->response_reader, &cli->listener, dds_DATA_AVAILABLE_STATUS);
  if (ret != dds_RETCODE_OK) {
