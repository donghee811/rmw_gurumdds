  ...
  rmw_subscription_allocation_t * allocation)
{
  ...
  *taken = false;
  ...
  if (sample_info->valid_data && ros_message != NULL) {
...
static void
datareader_callback(const dds_DataReader * a_reader) {
  ...
  rmw_ret_t rmw_ret = _take(gurum_gurumdds_identifier, data->subscription, nullptr, &taken, nullptr, null_allocation);
  if (rmw_ret != RMW_RET_OK) {
    RMW_SET_ERROR_MSG("_take failed");
    return;
  }
  ...
  const rmw_subscription_options_t * subscription_options)
{
  ...
  subscriber_info->user_callback_data.subscription = subscription;
  subscriber_info->listener.on_data_available = datareader_callback;
  listener_set_event_callbacks(&subscriber_info->listener, nullptr);
  dds_DataReader_set_listener_context(subscriber_info->topic_reader, &subscriber_info->user_callback_data);
  dds_DataReader_set_listener(subscriber_info->topic_reader, &subscriber_info->listener, dds_REQUESTED_DEADLINE_MISSED_STATUS |
...
{
  auto sub = static_cast<GurumddsSubscriberInfo *>(rmw_subscription->data);
  GurumddsUserCallback * data = &(sub->user_callback_data);
  dds_ReturnCode_t ret;

  std::lock_guard<std::mutex> guard(data->mutex);
  ret = dds_DataReader_set_listener(sub->topic_reader, &sub->listener, dds_DATA_AVAILABLE_STATUS |
  ...
  if (callback && data->unread_count) {
    dds_DataReaderQos dds_qos = {};
    ret = dds_DataReader_get_qos(sub->topic_reader, &dds_qos);
    if (ret != dds_RETCODE_OK) {
      RMW_SET_ERROR_MSG("subscription can't get data reader qos policies");
      return RMW_RET_ERROR;
    }

    size_t events = std::min(data->unread_count, static_cast<size_t>(dds_qos.history.depth));
